...
Temporal ERGMs (STERGMs) are an extension of ERGMs for modeling dynamic networks in discrete time.    Where an ERGM posit a single model for the _prevalence_ of ties in a cross-sectional single network, TERGMs instead posit two or more models for the _tie dynamics_ in a network over time, and each model is itself an ERGM.  
...
* Some new temporally-specific terms are available.

* The biggest difference is in the syntax we use to identify what kinds of changes we want to track: these are specified using the new "operator" terms available in version 4.0 of the `ergm` and `tergm` packages.  There are five temporal operators:  `Form()`, `Diss()`, `Persist()`, `Cross()` and `Change()`. We will explain their use in detail below.

TERGMs are a very general family of models.  Sub-classes within this family are defined by how tie changes are tracked and counted, and the model specification for each type of change.  We will focus on two important sub-classes in this tutorial: the joint model and the separable model.  These allow us to introduce all of the operator terms implemented in the `tergm` packages, and provide a good intuitive foundation for temporal model specification.
...
*  *The joint model* specifies a model for the starting network, and a model for the changes between time points.   The model for the starting network is essentially an ERGM -- it represents the distribution of ties in the initial, static network.  The model for the changes, because it is a single formula, requires the factors that that influence tie formation and tie dissolution to be the same. The change model coefficients represent the change in the prevalence of a configuration relative to the starting network: if negative the prevalence of that configuration is declining, if positive it is increasing.

* *The separable model* specifies a formula for tie formation, and a formula for tie dissolution.  With this class one can represent different factors influencing tie formation and tie dissolution, by choosing different terms in the two models.  The coefficients in the formation model represent how creating a configuration impacts the conditional log-odds of this tie forming (positive or negative), and the coefficients in the dissolution model represent that impact on tie persistence (again positive or negative). 
...
could be reduced by modifying some of `tergm`'s control parameters.  You can
