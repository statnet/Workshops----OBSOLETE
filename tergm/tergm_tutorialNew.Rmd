...
Many term names carry over from the `ergm` package, and they refer to the same configuration (e.g., `edges` refers to a tie between two nodes, and `degree(1) refers to nodes with degree equal to 1).  *However, the way the terms count these configurations is different, and depends on context.*

Some new temporally-specific terms are available for monitoring.

## TERGM Operators

The biggest difference you'll notice in specifying a TERGM is that this typically involves more than one formula.  The formulas are specified using the new "operators" available in version 4.0 of the `tergm` package.  
...
You can mix these operators in different ways, but no more than 2 operators can include the same model term, otherwise a model will be overspecified.
...
*  *The joint model* Joint models specify a process for tie dynamics using operators that are dependent within time steps; the `Cross() + Change()` model is the classice example.  Here, a single dyad change will affect both the `Cross()` cross-sectional network statistics and the changes counted in the `Change()` model statistics at each timestep. The terms in the two operators are correlated, so must be jointly evaluated at each step of estimation or simulation (hence the name).

* *The separable model* Separable models specify a process for tie dynamics using operators that are independent within time-step (though they may be dependent between time steps); the `Form() + Diss()` (or `Persist()`) model is the classic example.  Here, a single dyad change will affect either the `Form()` model statistics or the `Diss()/Persist()` model statistics at each timestep, but not both, because that dyad will either not have an edge or have an edge in the starting network.  So the terms in the two operators are uncorrelated, and can be separately evaluated at each step for estimation or simulation.

This distinction has important implications for the type of data that can be used to estimate a TERGM, so it is a key difference to keep in mind.

### The joint model

 -- To Do --
 
Imagine a time series of 5 networks, with identical cross-sectional structure.  The dynamics that could produce this pattern lie on a continuum.  At one end is a process with no change at all: every tie persists and every empty dyad stays empty.  At the other end is a process of maximal change:  every tie dissolves at each timestep, and a new set of the same number of ties is formed in the same configurations. 
 
 that's right. cross-sectional statistics (whether via Cross() operator or just bare statistics) as well as Change() violate separability between formation and dissolution.

They are also not separable from each other, either, because whereas a single-dyad change from t-1 to t will affect either Form statistic, Diss statistic, but not both (because that dyad will either not have an edge or have an edge in t-1, but not both), that dyad change will simultaneously affect both the cross-section of t and the change network from t-1 to t (in the same or opposite directions).
...
### `tergm` Model specification and syntax
...
For a call to `tergm`, there is one dynamic network, with two formulas. These are now passed as three separate arguments: 
...
### Notes on independence within, not between time steps
