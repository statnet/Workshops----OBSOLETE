...
As the name suggests, this package generalizes the `sna` package functionality for temporal network data.  It provides utilities for calculating a range of cross-sectional and temporal statistics.
...
The TERGM model specifications use the standard formula syntax (~ term1 + term2 ...), with terms that represent network statistics, just like we do for ERGMs,  the term names are the same as they are in an ERGMs, and they refer to the same configuration (e.g., `edges` refer to a tie between two nodes, and `degree(1) refers to nodes with degree equal to 1).  However, the way the terms count these configurations is different, and depends on context. 

This is a very general class of models, with two important sub-classes that we will focus on here: the joint model and the separable model.

The joint and separable sub-classes both specify a pair of equations, but they differ in how they represent the change dynamics.  But the joint and separable sub-classes differ in how and what those terms count.  

*  *The joint model* specifies a model for the starting network, and a model for the changes at later time points.   The model for the starting network is essentially an ERGM -- it represents the distribution of ties in the initial, static network.  The model for the changes, because it is a single formula, requires the factors that that influence tie formation and tie dissolution to be the same. The change model coefficients represent the change in the prevalence of a configuration relative to the starting network: if negative the prevalence of that configuration is declining, if positive it is increasing.
