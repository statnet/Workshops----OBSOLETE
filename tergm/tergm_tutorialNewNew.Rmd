...
* the formula, but this now will typically include more than one Operator (each with it's own set of terms), and 
...
In the following code, we pass five arguments: the network list that we want to model (`samp.list`), the formation formula, the dissolution formula, the fitting algorithm (CMLE since we have a network panel), and the list of time slices we wish to model:

```{r sampfit1, results = "hide", message = FALSE}
# samp.fit <- stergm(samp.list,
