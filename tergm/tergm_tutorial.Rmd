...
_This tutorial is a joint product of the Statnet Development Team:_ 
...
The temporal modeling software we will be using is authored by Pavel Krivitsky (`tergm`) and Skye Bender de-Moll (`networkdynamic`, `tsna` and `ndtv`).
...
Both models are ERGMs, and they are "separable" in the sense that they assume formation is independent of dissolution _within time step_, but Markov dependent between steps.  This allows the factors that influence formation to be different than those that influence dissolution, so
...
The formation equation can represent this with the ergm term "degree(1)", which counts the number of nodes in the network of degree exactly 1.  The coefficient on this term would be positive, because new ties are more likely to form for singles (thus increasing the count of degree 1 nodes) and less likely for those already partnered (which would decrease the count of degree 1 nodes). 

The dissolution equation can represent this with the ergm term "concurrent", which counts the number of nodes of degree 2 or more. The coefficient for this term would be negative because the ties are relatively less likely to persist if they maintain the current number of nodes of degree 2 or more, when they could bring it down by dissolving.

Finally, remember that the _s_ in _stergm_ stands for separable; this refers to the fact that the two models assume that relational formation and dissolution occur independently of one another within a time step.  We explore how this works and its implications in the examples.  But effectively, at each timestep, we are sending off all empty dyads to be considered for formation, and sending off all the paired dyads to be considered for dissolution, and the two processes proceed independently.  
...
One additional specification is _required_ for a `stergm` as shown above:
...
The cannonical form of data for modeling dynamic network processes consists of observations of a complete network at two or more points in time on the same node set.  Many classic network studies were of this type, and this remains a gold standard for temporal network data collection.
...
So, all else equal, a relationship is much more likely to form if it will close a mutual pair; the conditional log-odds are increased by 2.0, which translates to an increase in the conditional odds of 7.4 (note that the change statistic can equal only $0$ or $1$ in this case).  The effect on persistence is also positive, but less strong and not statistically signifiant; the point estimate is an increase of 0.8 in the conditional log-odds of persistence, a bit more than double the conditional odds.
...
To include only a subset of times from a network series, you can alter the `times` argument:
...
Now, imagine a different scenario in which we have observed a single cross-sectional network, and the duration of ties. 
This kind of data can be collected in a cross-sectional survey, so it
is easier to obtain.  

Can we estimate the formation and dissolution models with a single cross-sectional network?  Yes, by making an equilibrium assumption.
...
The duration information required here can be obtained by
retrospective reporting on the duration of relationships.  
This can come from the same cross-sectional study, 
or, if necessary, from a completely different data source.

We will demonstrate the latter by creating a toy dataset, based on 
Padgett's cross-sectional `flobusiness` network, using a made up mean tie
...
at all times). We can model heterogeneous durations as we saw above,
but this simple case will suffice to demonstrate the basic modeling
tasks.


__First__, we specify the formation and dissolution models.

We begin by assuming a formation model identical to the model we fit in the cross-sectional case:

`formation = ~edges+gwesp(0,fixed = T)`

For our dissolution model, we are assuming a simple homogenous process,
which corresponds to a model with only an edgecount term in it.  The coefficient on this term represents the conditional log-odds of tie persistence at each step, and the inverse is the mean log duration.
...
__Third__, we choose our equilibrium target statistic prevalence.  
...
* If the _values_ to be targeted are different than the values of
...
These look ok, though not great.  Chances are we would do better by modifying some of `stergm`'s control parameters, increasing the `mcmc.interval` to reduce the serial correlation evident in the traceplots.  But let's explore the fit object to see what we have:
...
is guaranteed to reproduce, on average, the cross-sectional network statistics we have specified.  
