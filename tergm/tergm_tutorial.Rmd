...
on principled statistical estimation of the model parameters that allows for hypothesis testing and inference at the model building stage. 
...
In addition to the two equations, on more specification is _required_ 
...
`round(samp.fit$formation.fit$coef[[2]],1)` 
which translates to an increase in the conditional odds of 
`exp(round(samp.fit$formation.fit$coef[[2]],1))`
...
`round(samp.fit$dissolution.fit$coef[[2]],1)`
...
an estimate of the $duration$, so we can use `stergm` to solve for the $incidence$ in the formation model that would be consistent with
...
Let the probability of persistance from one time step to the next for actor pair $i,j$ be $p_{ij}$, 
...
This can be one of three things:  the statistics in the formation model, the statistics in the dissolution model, or a different set of statistics altogether.  If the formula is identical to either the formation or dissolution model, the user can simply pass the string `"formation"` or `"dissolution"`, respectively. If one is specifying `targets = "formation"`, dissolution should be an offset, and vice versa. 

If the _values_ of the target statistics are different 
than the values of
the sufficient statistics in the observed cross-sectional network, then those values can be passed with the argument `target.stats`. 
...
These look ok, though not great.  Chances are we would do better by modifying some of `stergm`'s control parameters, increasing the `mcmc.interval` to reduce the serial correlation evident in the traceplots.  You should try that.  Here, we'll move on to explore 
the fit object to see what we have:
...
that, conditional on the given dissolution model (and it's offsets), yields simulated targets that match (in expectation) those observed in the flobusiness network.
...
is guaranteed to reproduce, on average, the full joint distribution of the cross-sectional network statistics and partnership durations we 
...
Although right-censoring is present for some edges in our simulation, with a mean duration of 10 time steps and a simulation 1000 time steps long, the effect of censoring on our observed mean duration should be small.  We'll show that this produces the same result as the edgeDuration function.
...
(with dissolution model offsets that contain a subset of terms 
...
(1) Fit the terms in the formation model as a static ERGM on the cross-sectional network; then 

(2) Subtract the values of the dissolution offsets from the estimated coefficients for the corresponding terms in the cross-sectional ERGM. The result is a vector of parameter values that form a reasonable place to start the MCMC chain for the estimation of the formation model.  

This is in fact exactly what the `stergm` estimation code does by default for this type of model.

When mean relational duration is very long, this approximation is so good that it may not be necessary to run a STERGM estimation at all. This can be quite useful, since models with long mean duration are precisely the ones that 
...
To be able to take advantage of this approximation, it is necessary for the terms in your dissolution model to be a subset of the terms in your formation model. 
