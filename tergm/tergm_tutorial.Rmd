...
##2. Temporal network data: Some considerations
...
* Network census panel data (with 2 or more panels)
...
## 3. Exploring temporal network data

### networkDynamic:  an object class for storing temporal network data, and a package for working with it.
...
for visualizing changes in network structure and attributes over time. `ndtv` has its own tutorial, with materials currently at https://statnet.csde.washington.edu/trac/wiki/Sunbelt2014#WorkshopMaterials
...
## 4. Modeling temporal network data with STERGMs
...
## 5. Example data analyses

We'll work through three examples here, first with network panel data, next with cross-sectional data plus duration information, and the third is an
approximation used in the cross sectional case when durations are very
long.
...
Let us return to the Sampson monastery data.  We have already constructed the networkDynamic object.  Now we must decide on models for the formation and dissolution.
...
fact that, at equilibrium, $prevalence = incidence * duration$.  
...
Our dissolution model homogenous, so we further define $p_{ij}=p$ for all $i,j$ and $q_{ij}=q$ for all $i,j$. Then:
...
These look ok, though not great.  Chances are we would do better by modifying
some of `stergm`'s control parameters.  But let's explore the fit object to see what we have:
...
We can use `ndtv` to visualize the simulated network time series:
...
Although right-censoring is present for some edges in our simulation, with a mean duration of 10 time steps and a simulation 1000 time steps long, its effect on our observed mean duration should be small.
...
## 6. Notes on independence within, not between time steps
