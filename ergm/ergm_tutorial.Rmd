...
This is a *very brief* overview of the modeling framework, as the primary purpose of this tutorial is to show how to implement this type of analysis using the `statnet` software tools.  For more detail (and to really understand ERGMs) please see the [references](ergm_tutorial.html#references) at the
...
* Obtain maximum-likehood estimates for the parameters of a specified model for a given data set; 
...
This expression shows that the coefficient $\theta$ can be interpreted as the log-odds of an individual tie conditional on all others. 

### Model terms in an ERGM

The model terms $g(y)$ in an ERGM are *network statistics*, typically counts of network configurations that we hypothesize may be more or less common than what would be expected in a simple random graph (where all ties have the same probability).  For example, specific degree distributions, triad configurations, or homophily on nodal attributes.  Unlike traditional
...
of dyad states (or of products of dyad states) that represent 
...
& = & `r round( exp(flomodel.01$coef)/(1 + exp(flomodel.02$coef)), 2)`
\end{eqnarray*}
}
$$
