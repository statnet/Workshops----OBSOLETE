...
$$ \mbox{my.network} \sim \mbox{my.vector.of.model.terms} $$
...
MUST CHANGE THIS.  Don't need ergm-terms-help for the second time?
For a list of available terms that can be used to specify an ERGM, type:

```{r ergm-terms-help2,eval=FALSE}
help('ergm-terms')
```
...
Writing up new `ergm` terms requires some knowledge of 
C and the ability
...
Voil\`a. Of course, yours will look somewhat different.
...
This turns out to be one of the classic degenerate model specifications,
...
Better, but not great. Here we'll change two things.  First, we'll add some more reasonable terms to the model, and 
second, we'll make the fitting algorithm more robust by making the MCMC simulation longer.
...
HOWEVER:  This volume was written in 2008.  The statnet code base has evolved considerably since that time, so some of the syntax specified in the articles may no longer work (in most cases because it has been replace with something better).
