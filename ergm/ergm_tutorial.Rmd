...
The statistics $g(y)$ can be thought of as the "covariates" in the model.  The represent network features like density, homophily, triads, etc.  In one sense, they are like covariates you might use in other statistical models.  But they are different in one important respect:  they are functions of the network itself -- defined by sums and products of the dyads in the network -- so they are not measured by a question you include in a survey (e.g., the income of a node), but instead need to be computed on the specific network you have, after you have collected the data.

As a result, every term in an ERGM must have an associated algorithm for computing its value.  The `ergm` package in `statnet` includes about 150 term-computing algorithms.  We will explore some of these terms in this 
...
You can get the list of all terms, and the syntax for using them, by typing:
...
* $\delta(y_{ij})$ is a vector of the "change statistics" for each model term.  The change statistic records how $g(y)$ term changes if the $y_{ij}$ tie is toggled on or off.  So:
...
This expression shows that the coefficient $\theta$ can be interpreted as the log-odds of an individual tie conditional on all others.  For each term you have in the model, the coefficient is multiplied by the number of configurations that tie will create (or remove) for that specific term.  We will see exactly how this works in the sections that follow. 
...
* if one triangle: 
$`r round(flomodel.02$coef[[1]],2)` + `r round(flomodel.02$coef[[1]],2)` = `r round(flomodel.02$coef[[1]] + flomodel.02$coef[[1]],2)`$

* if two triangles: 
$`r round(flomodel.02$coef[[1]],2)` + 2 \times`r round(flomodel.02$coef[[1]],2)` = `r round(flomodel.02$coef[[1]] + 2*flomodel.02$coef[[1]],2)`$
