...
user's ability to create terms even further through the use of operators.  As mentioned
[above in Section 1](ergm_tutorial.html#the-model-statistics-gy-ergm-terms), it is possible to search for specific
topics using `search.ergmTerms` or to obtain
help on the full array of terms using `help("ergm-terms")`.  
...
the 'Specifications' paper in the [*Journal of Statistical Software v24(4)*](http://www.jstatsoft.org/v24/i04)
...
There is a `statnet` package, `ergm.userterms`, that provides the utilities needed to write new
...
to build R from source.  While the latter is covered in the tutorial, the many environments for building R and the
rapid changes in these environments make these instructions obsolete quickly. 
...
`ergm` version 4.0, the most current paper describing the capabilities of the `ergm` package is
[Krivitsky et al (2021)](https://arxiv.org/abs/2106.04997).

so some of the syntax specified in the articles may no longer work (in most cases because it has been replace with something better).
