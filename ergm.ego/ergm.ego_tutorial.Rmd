...
so we would expect a network of 20 people to have 80 ties.  As with everything network, things are more complicated for
...
# se for the diff in coef, from MCMC only
# note use of mcmcse function we defined
sediff = sqrt(se.coef(fit, "estimation")^2 + se.coef(fit.ego, "estimation")[-1]^2)
...
coefs1 <- tibble::rownames_to_column(data.frame(coef(fit))) %>%
  ...
  mutate(Zdiff = (coef.fit. - coef.fit.ego.)/sediff)

round_df(coefs1, 3) # note use of round_df function we defined
```

Here, to compare the coefficient estimates, we are constructing a z-score
for the difference, based on the MCMC error in each run.  Since this is
an egocentric *census*, there is technically no error in the sufficient
statistics for the egocentric model terms, so the only error in estimation
comes from the MCMC algorithm.  To reduce the difference, we could
increase the size of our MCMC sample (but we'll move on for now).
...
sediff = sqrt(se.coef(fit, "estimation")^2 + se.coef(fit.ego.sampN., "all")[-1]^2)
...
#  left_join(tibble::rownames_to_column(data.frame(coef(fit.ego))))  %>%
  ...
  mutate(Zdiff = (coef.fit. - coef.fit.ego.sampN.)/sediff)
...
```
```{r, message=SHOW_MSG}
fmh.egosamp.25 <- sample(fmh.ego, round(0.25*N), replace=TRUE)

fit.ego.samp.25 <- ergm.ego(fmh.egosamp.25 ~ edges + degree(0) + nodefactor("Race")
...
coefs4 <- tibble::rownames_to_column(data.frame(coef(fit)))%>%
  ...
  mutate(Zdiff = (coef.fit.ego.samp.25. - coef.fit.ego.samp.25strat.)/zdiffse) %>%
  select(-coef.fit.)
