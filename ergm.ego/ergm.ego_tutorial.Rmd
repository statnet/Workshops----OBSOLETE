...
To look at the mixing matrix, we use the generic function `mixingmatrix` which will dispatch to the
method specific to egodata called `mixingmatrix.egodata`.  The function will return either the crosstabulation
...
using another generic function `degreedist` which will dispatch to the `egodata` specific method
