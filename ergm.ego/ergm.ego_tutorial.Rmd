...
# remotes::install_github("statnet/ergm.ego@master",
#                         lib = master_lib)
...
The framework for inference relies on two basic properties of 
...
adjusted for network size and some
types of observable discrepancies. 
...
for the ERGM estimation: instead of using $|N|$ we use $|N'|$.

The resulting values are the target statistics that we pass to `ergm`.

If we wanted to obtain population estimates from `ergm.ego` for a population
with a known size $N$, we would fit the model with an
offset of $\log(N/{N'})=\log(N) - \log(N')$.  In general, however, we do not know $N$.  So we often
fit with an offset of $-\log(N')$, which will return
...
* `egor` objects for storing the original data (the analog to `network` objects in `ergm`), 
...
### 5a. Data construction and manipulation:   `egor` objects and their related functions

An `egor` object comprises a `tibble` (a data frame with slightly different behavior), each of whose rows represents an ego, and which has a special column, `.alts`, containing more `tibble`s with alters and alter attributes. (Another column, `.aaties`, contains alter--alter nominations; its format is subject to change.)

It also has attributes `ego.design`, a `survey.design` object specifying how the egos were selected and `alter.design`, a list specifying things like maximum numbers of alters.

`egor` objects can be constructed from
...
: * optionally, arguments to `svydesign()` function from the `survey` package, specifying the go design

`CSV` files (`read.*()`)
: `egor` package has utilities for loading egocentric data from a variety of formats; currently buggy

`network` objects (`as.egor()`)
: The function will create an egocentric census from an existing network. Mostly used for testing and pedagogical purposes.

`egodata` objects (`as.egor()`)
: `egodata` is deprecated, so only a method to convert `egodata` objects to `egor` exists.


The `as.network` method for `egor` will transform an egor 
...
Querying `egor` objects

`egor` is just a fancy data frame, so standard indexing and subsetting Just Works (TM).

Some special methods are also implemented

* `sample` -- a `sample` method for `egor` object with the target number of egos
* `subset` -- allows you to define a condition for the subset of egos, alters, or alter--alter ties to choose
* `na.omit` -- returns an `egodata` object without NAs in their 
(or their alter's) columns
* `ego.design` can be used to query or set the sampling design.
* `alter.design` can be used to query or set the alter selection information.
...
terms, mixing terms, and degree distribution terms, and triadic terms.  The terms actually available
in `ergm.ego` are limited in addition to those that have been coded up.  At this
point this is a small subset (n=14) of the terms currently available for `ergm`,
...
Triadic terms include, `esp`, `gwesp`, `transitiveties` and `cyclicalties`, but only if alter--alter ties have been observed.
...
### 6a. The `egodata` object

We'll start by using one of the built-in datasets in the `ergm` package, and transform it to an `egodata` object so we can see what that looks like.
...
* The standard network edgelist contains one unique record for each edge; both ego and alter ID may appear more than once (depending on their degree), but the link is only represented once.  

* The alter table in this `egodata` object is a different type of edgelist, as it is "egocentric."  When derived from a network census, as we did above, each tie will be represented *twice*: once with the first node as the ego, and once with the other node as the ego.  As a result, the number of times alters will appear in the `.altID` list is equal to their degree, as is the number of times their ID will appear in the `.egoID` list.  When the alter table is derived from an egocentric *sample* of a network, it is likely that each tie will only be represented once, unless some of the alters are sampled as egos.  When the sampling fraction is small (e.g., for the General Social Survey friendship network data), it is very unlikely that alters will be sampled.
...
Since the egodata tables are simple rectangular matrices, it's easy to read in external data and use it to construct an egodata object.  You just need to make sure that the structure of the external file is consistent with the structure of the tables we looked at above.

To demonstrate we will construct an egodata object derived our `mesa.ego` data that has the features of an egocentrically sampled data set: the alters are not uniquely identified.
...
`egor` object in a later section.
...
The `degreedist` method for `egor` also has an argument that lets you overplot the expected
...
uncertainty is being used to produce the standard errors for our coefficients.
...
(Note that we have implicitly used simulate already -- it's the basis of the GOF 
results)
...
```{r}
fit.ergm <- ergm(fmh ~ edges + degree(0:3) 
                 + nodefactor("Race") + nodematch("Race")
                 + nodefactor("Sex") + nodematch("Sex") 
                 + absdiff("Grade"))
data.frame(coef(fit.ergm))
```
...
egofit <- ergm.ego(fmh.ego ~ edges + degree(0:3) 
                   + nodefactor("Race") + nodematch("Race")
                   + nodefactor("Sex") + nodematch("Sex") 
...
model.se <- function(fit) sqrt(diag(vcov(fit, sources="model"))) 

# Parameters recovered:
coef.compare <- data.frame(
  "Full nw est" = coef(fit.ergm), 
  "Ego census est" = coef(egofit)[-1],
...
egofitN <- ergm.ego(fmh.egosampN ~ edges + degree(0:3) 
                    + nodefactor("Race") + nodematch("Race") 
                    + nodefactor("Sex") + nodematch("Sex")
                    + absdiff("Grade"),
                    popsize=N)
...
coef.compare <- data.frame(
  "Full nw est" = coef(fit.ergm), 
  "Ego sample N est" = coef(egofitN)[-1],
...
se.compare <- data.frame(
  "Full nw SE" = model.se(fit.ergm), 
  "Ego census SE" =model.se(egofit)[-1], 
  "Ego sample N SE" = model.se(egofitN)[-1])
...
egofitN4 <- ergm.ego(fmh.egosampN4 ~ edges + degree(0:3) 
                    + nodefactor("Race") + nodematch("Race") 
                    + nodefactor("Sex") + nodematch("Sex")
                    + absdiff("Grade"),
                    popsize=N)
...
coef.compare <- data.frame(
  "Full nw est" = coef(fit.ergm), 
  "Ego sampl N/4 est" = coef(egofitN4)[-1],
...
se.compare <- data.frame(
  ...
  "Ego census SE" =model.se(egofit)[-1], 
  "Ego Sample N SE" = model.se(egofitN)[-1],
  "Ego Sample N/4 SE" = model.se(egofitN4)[-1])
...
Suppose that we oversampled the minority groups by a factor of 4. We
survey a non-white student 4 times more likely than in a simple random
sample.
...
head(fmh.egosampN4w)
egofitN4w<-ergm.ego(fmh.egosampN4w ~ edges + degree(0:3) 
                    + nodefactor("Race") + nodematch("Race") 
                    + nodefactor("Sex") + nodematch("Sex")
                    + absdiff("Grade"),
                    popsize=N)
...
se.compare <- data.frame(
  "Full nw SE" = model.se(fit.ergm),
  "Ego Sample N/4 SE" = model.se(egofitN4)[-1],
  "Oversampled SE" = model.se(egofitN4w)[-1])
...
* Support for alter--alter nominations, integrated with `egor` (available at `tilltnet/egor`).
...
We are working on a paper describing the package for the *Journal of Statistical Software* with a target submission date in 2017.
