...
param.compare <- cbind(coef(fit.ergm), 
                       coef(egofit)[-1],
                       (coef(fit.ergm)-coef(egofit)[-1])/modse(egofit)[-1])
colnames(param.compare) <- c("Full nw est", 
                             "Ego census est", 
                             "diff Z")
...
# compare the s.e.'s
se.compare <- cbind(modse(fit.ergm), 
                    modse(egofit)[-1], 
                    modse(egofitN)[-1])
colnames(se.compare) <- c("Full nw SE", 
                          "Ego census SE", 
                          "Sample N SE")
round(se.compare, 3)
